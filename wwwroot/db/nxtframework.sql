...
CREATE TABLE IF NOT EXISTS `nxtframework`.`nxt_order_form_refund` (
  ...
  `transaction_id` BIGINT(20) NULL,
  `status` INT NOT NULL COMMENT '状态（-1:拒绝退款 0:已申请 1:完成 2:等用户发货 3:收到货退款 4:收到货有问题，请修改金额 5:用户已寄出物品）',
...
-- -----------------------------------------------------
-- Table `nxtframework`.`nxt_user_verifiy`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nxtframework`.`nxt_user_verifiy` ;

CREATE TABLE IF NOT EXISTS `nxtframework`.`nxt_user_verifiy` (
  `id` BIGINT(20) NOT NULL AUTO_INCREMENT,
  `type` INT NOT NULL COMMENT '-1：修改绑定 1：绑定账户 2：找回密码 3：提现验证',
