...
CREATE TABLE IF NOT EXISTS `nxtframework`.`nxt_order_form` (
  ...
  `amount_initial` BIGINT(20) NOT NULL COMMENT '最初总金额（放大100倍）',
  `amount_discount` BIGINT(20) NOT NULL COMMENT '优惠金额（放大100倍）',
  `amount_finally` BIGINT(20) NOT NULL COMMENT '最终总金额（放大100倍）',
  ...
  `delivery_type` VARCHAR(255) NOT NULL COMMENT '配送方式',
...
CREATE TABLE IF NOT EXISTS `nxtframework`.`nxt_order_form_product` (
  `id` BIGINT(20) NOT NULL COMMENT '订单管理商品',
  `order_form_id` BIGINT(20) NOT NULL,
  `product_id` BIGINT(20) NOT NULL COMMENT '产品id',
  ...
  `is_refund` INT(11) NOT NULL COMMENT '是否退货了（0否 1是）',
  `commission_rate` BIGINT(20) NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `nxtframework`.`nxt_commission` (
  ...
  `commission_amount` VARCHAR(45) NOT NULL COMMENT '佣金（放大100倍）负数表示退佣金',
  `dateline_create` BIGINT(20) NOT NULL COMMENT '交易创建时间',
  `dateline_end` BIGINT(20) NULL COMMENT '交易完成时间',
  `deal_status` INT NOT NULL COMMENT '交易状态 (0:进行中 1:完成）',
  `is_refund` INT NOT NULL COMMENT '是否退款（0否 1是）',
  PRIMARY KEY (`id`),
...
-- -----------------------------------------------------
-- Table `nxtframework`.`nxt_order_form_delivery_refund`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `nxtframework`.`nxt_order_form_delivery_refund` (
)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `nxtframework`.`nxt_shopping_cart` (
  `id` BIGINT(20) NOT NULL COMMENT '购物车，仅用token也可以查询（访客）',
