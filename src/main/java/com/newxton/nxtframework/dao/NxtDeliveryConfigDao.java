package com.newxton.nxtframework.dao;

import com.newxton.nxtframework.entity.NxtDeliveryConfig;
import org.apache.ibatis.annotations.Param;
import java.util.List;

/**
 * (NxtDeliveryConfig)表数据库访问层
 *
 * @author makejava
 * @since 2020-11-02 19:03:31
 */
public interface NxtDeliveryConfigDao {

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    NxtDeliveryConfig queryById(Long id);

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<NxtDeliveryConfig> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


    /**
     * 通过实体作为筛选条件查询
     *
     * @param nxtDeliveryConfig 实例对象
     * @return 对象列表
     */
    List<NxtDeliveryConfig> queryAll(NxtDeliveryConfig nxtDeliveryConfig);

    /**
     * 新增数据
     *
     * @param nxtDeliveryConfig 实例对象
     * @return 影响行数
     */
    int insert(NxtDeliveryConfig nxtDeliveryConfig);

    /**
     * 修改数据
     *
     * @param nxtDeliveryConfig 实例对象
     * @return 影响行数
     */
    int update(NxtDeliveryConfig nxtDeliveryConfig);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 影响行数
     */
    int deleteById(Long id);

}